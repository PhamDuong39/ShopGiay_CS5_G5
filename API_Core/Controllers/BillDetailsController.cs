...
namespace API_Core.Controllers
{
    ...
    public class BillDetailsController : ControllerBase
    {
        ...
        {
            if (_ishoesrepos.GetAll().FirstOrDefault(p => p.Id == IdShoeDetail).AvailableQuantity < quantity)
            {
            ...
            else
            {
                ...
                if (_irepos.GetAll().Any(p => p.IdBill == IdBill && p.IdShoeDetail == IdShoeDetail))
                {
                    ...
                    if (quantity > _ishoesrepos.GetAll().FirstOrDefault(p => p.Id == IdShoeDetail).AvailableQuantity)
                    {
                        return "Số lượng không đủ";
                    }
        ...
        {
            var obj = _irepos.GetAll().FirstOrDefault(p => p.Id == id);
            //neu xoa billdetail thi update lai so luong trong kho
            ShoeDetails sd = _ishoesrepos.GetAll().FirstOrDefault(p => p.Id == obj.IdShoeDetail);
            sd.AvailableQuantity += obj.Quantity;
            _ishoesrepos.Update(sd);
